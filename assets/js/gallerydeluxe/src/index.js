...
let GalleryDeluxe = {
	init: async function () {
		...
		captionEl.id = 'gd-modal-caption';
		captionEl.classList.add('gd-modal-caption');
		...
		const openActiveImage = () => {
			...
			if (activeImage) {
				...
				if (activeImage.title && activeImage.title !== activeImage.name) {
					captionEl.textContent = activeImage.title;
